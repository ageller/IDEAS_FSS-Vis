...
function createLegend(info){
	...
	legend.append("text")
		...
		var header = d3.select('#'+'legendcontainer').append('header')
			...
			.style('left', (leftLocation+(width/2)-70)+'px')
}
...
d3.dsv(' ', '2001ig_alpha_copy.txt')
	.then(function(d) {
		...
		var axes3 = init(d, 'container3', 30, 800, 't', 'alpha1', 'alpha2', 'SED', 'MJD Since Explosion', 'Value', alphaCharacter+' Values', xdomain=null, [-20,20]);
...
d3.dsv(' ', '2001ig_peak_copy.txt')
	.then(function(d) {
		var axes4 = init(d, 'container4', 720, 800, 'freq', 'flux', 'SED', 'SED', 'Frequency (GHz)', 'Flux (microJy)', 'Peak Values', xdomain=null, ydomain=null, log=true) ;
...
.then(function(d) {
	var axes = init(d, 'container', 30, 265, 'freq', 'flux', 'SED', 'SED', 'Frequency (GHz)', 'Flux (microJy)', 'Individual Fits', xdomain=null, [1,1e+5], log=true);
	axes5 = init(d, 'container5', 1410, 570, 'freq', 'flux', 'SED', 'SED', 'Frequency (GHz)', 'Flux (microJy)', 'Individual SED', [1e-2, 1e+4], [1,1e+5], log=true);
...
.then(function(d) {
	var axes2 = init(d, 'container2', 720, 265,  'freq', 'flux', 'SED', 'SED', 'Frequency (GHz)', 'Flux (microJy)', 'SEDs Fit Together', xdomain=null, [1,1e+5], log=true);
