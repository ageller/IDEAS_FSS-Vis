...
import resources.polyhedra as polyhedra

#grabbed this online somewhere
from resources.eulerangles import euler2mat
...
class runPyOpenGL(object):
	...
	def setLighting(self):
		...
		# Set light model
		ambient_color = np.array(self.ambient, 'f')
		glLightfv(GL_LIGHT0, GL_AMBIENT, ambient_color)
		glLightModelfv(GL_LIGHT_MODEL_AMBIENT, ambient_color)
	
		
		# Set position and color of light
		direction = np.array([self.lightX, self.lightY, self.lightZ, 0.0], 'f')
		glLightfv(GL_LIGHT0, GL_POSITION, direction)
		color = np.array(self.diffuse, 'f')
	...
	def getIsoSphere(self):
		...
		for x in range(self.Nsubdivisions):
...
#from here : https://www.gamedev.net/topic/116339-request-for-help-texture-mapping-an-icosahedron/
	...
	def keyPressed(self, *args):
		...
		"""
		# If q or escape is pressed, kill everything.
	...
	def sortZ(self):
		...
		MR = euler2mat(x = -self.xrot*np.pi/180., y = -self.yrot*np.pi/180., z = self.zrot*np.pi/180.)
		xyz_rot = [np.dot(MR, [x - self.centerX, y - self.centerY, z - self.centerZ]) for (x,y,z) in zip(self.objX, self.objY, self.objZ)]
		x_rot = np.array([i[0] for i in xyz_rot])
		y_rot = np.array([i[1] for i in xyz_rot])
		z_rot = np.array([i[2] for i in xyz_rot])
		srtz = np.argsort(z_rot)[::-1]
